...
def sentencify(ascii_art_list: list, new_width, class_name):
    ...
    formatting_ascii = re.sub(r'\n', f'\n<{class_name}_start>', skipping_return)
    return formatting_ascii

def image_to_ascii(image, ascii_chars=DEFAULT_ASCII_CHARS, class_name="", size=40):
    """Convert the image to ASCII art."""
    pixels = image.getdata()
    pixel_range = len(ascii_chars) - 1
    characters = "".join([ascii_chars[pixel_value * pixel_range // 255]for pixel_value in pixels])
    return sentencify(characters, size, class_name)
...
# %%
def generate_ascii_art(image, class_name, output_width, ascii_chars=DEFAULT_ASCII_CHARS):
    """Generate ASCII art from an image."""
    image = resize_image(image, output_width)
    ascii_art = image_to_ascii(image, ascii_chars, class_name, output_width*2)
    return ascii_art
...
def process_batch(args):
    ...
    for sample in batch:
        ...
        ascii_30.append([generate_ascii_art(input_image, class_name, 25, ascii_chars), class_name, sample['name']])
        ascii_60.append([generate_ascii_art(input_image, class_name, 40, ascii_chars), class_name, sample['name']])
